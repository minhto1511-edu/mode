    ...
    <head>
        ...
        <script>
            function changeDirect() {
                window.location = 'staff?action=add';
            }

            function edit(username) {
                window.location = 'staff?action=edit&user=' + username;
            }

            function deleteUser(username, fullname, currentuser) {
                if (username != currentuser) {
                    if (confirm("Bạn có muốn xóa người dùng " + fullname + "?")) {
                        window.location.href = 'staff?action=delete&user=' + username;
                    }
                } else {
                    alert("Bạn không thể xóa account của bạn!");
                }
            }
    ...
    <body>
        <div class="container">
            ...
            <section class="main">
                ...
                <div class="add-data">
                    <button onclick="changeDirect()">Nhập dữ liệu</button>
                </div>
                </br>
                ...
                <table border="1">
                    ...
                    <tbody>
                        <c:forEach items="${requestScope.dataUser}" var="i">
                            <tr>
                                <td>${i.userId}</td>
                                <td>${i.username}</td>
                                <td>${i.role}</td>
                                ...
                                <td class="action-buttons">
                                    <button class="btn btn-edit" onclick="edit('${i.username}')">Edit</button>
                                    <button class="btn btn-delete" onclick="deleteUser('${i.username}', '${i.fullname}', '${sessionScope.user.username}')">Delete</button>
                                </td>
                            </tr>
                        </c:forEach>
